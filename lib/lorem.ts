...
function pick<T>(items: T[], random: () => number = Math.random): T { return items[Math.floor(random() * items.length)]; }
function nextWord(random: () => number = Math.random) { return pick(vocabulary, random); }
function sentenceWordCount(sentenceLength: SentenceLength, random: () => number) {
  const ranges = { short: [5, 11], mixed: [9, 20], long: [16, 30] } as const;
  const [minimum, maximum] = ranges[sentenceLength];
  return minimum + Math.floor(random() * (maximum - minimum + 1));
}
function sentence(wordCount: number, random: () => number) {
  const words = Array.from({ length: wordCount }, () => nextWord(random));
  words[0] = words[0][0].toUpperCase() + words[0].slice(1);
  if (words.length > 10 && random() > .45) words[Math.floor(words.length / 2)] += ",";
  return words.join(" ") + ".";
}
function generatedSentence(sentenceLength: SentenceLength, random: () => number) {
  return sentence(sentenceWordCount(sentenceLength, random), random);
}
function paragraph(sentenceLength: SentenceLength, random: () => number) {
  return Array.from({ length: 3 + Math.floor(random() * 4) }, () => generatedSentence(sentenceLength, random)).join(" ");
}
function withOpening(text: string, start: boolean) { return start ? `${classic} ${text}`.trim() : text; }

function exactCharacterTail(count: number, start: boolean, random: () => number) {
  ...
  reachable[0] = true;
  reachable[1] = true;
  for (let n = 2; n <= remaining; n++) reachable[n] = vocabulary.some(word => n >= word.length + 1 && reachable[n - word.length - 1]);
  let left = remaining;
  while (left > 1) {
    const options = vocabulary.filter(word => left >= word.length + 1 && reachable[left - word.length - 1]);
    if (!options.length) break;
    const word = pick(options, random);
    result += " " + word;
    left -= word.length + 1;
  }
  if (left === 1) result += ".";
...
function generateCharacters(count: number, start: boolean, sentenceLength: SentenceLength, random: () => number) {
  if (count <= 8) return exactCharacterTail(count, start, random);
  const sentences: string[] = [];
  let used = 0;
  if (start && count >= classic.length) {
    sentences.push(classic);
    used = classic.length;
  }
  while (count - used > 10) {
    const candidate = generatedSentence(sentenceLength, random);
    const separator = sentences.length ? 1 : 0;
    if (used + separator + candidate.length > count) break;
    sentences.push(candidate);
    used += separator + candidate.length;
  }
  const remaining = count - used - (sentences.length ? 1 : 0);
  if (remaining > 0) sentences.push(exactCharacterTail(remaining, !sentences.length && start, random));
  return sentences.join(" ").slice(0, count);
}

export function generateLorem(unit: Unit, amount: number, start = true, sentenceLength: SentenceLength = "mixed", random: () => number = Math.random): string {
  const count = Math.max(1, Math.min(Math.floor(amount || 1), unit === "characters" ? 20000 : unit === "words" ? 4000 : 100));
  if (unit === "paragraphs") {
    const paragraphs = Array.from({ length: count }, () => paragraph(sentenceLength, random));
    if (start) paragraphs[0] = withOpening(paragraphs[0], true);
    return paragraphs.join("\n\n");
  }
  if (unit === "sentences") {
    const sentences = Array.from({ length: count }, () => generatedSentence(sentenceLength, random));
  ...
  if (unit === "words") {
    ...
    while (remaining > 0) {
      const length = Math.min(remaining, sentenceWordCount(sentenceLength, random));
      const words = Array.from({ length }, () => nextWord(random));
      words[0] = words[0][0].toUpperCase() + words[0].slice(1);
      sentences.push(words.join(" ") + ".");
      remaining -= length;
    }
  ...
  return generateCharacters(count, start, sentenceLength, random);
}

export function resizeLorem(text: string, unit: Unit, oldAmount: number, newAmount: number, start: boolean, sentenceLength: SentenceLength = "mixed"): string {
  if (newAmount === oldAmount) return text;
  if (unit === "paragraphs") {
    const paragraphs = text.split("\n\n");
    return newAmount < oldAmount
      ? paragraphs.slice(0, newAmount).join("\n\n")
      : [...paragraphs, ...generateLorem("paragraphs", newAmount - oldAmount, false, sentenceLength).split("\n\n")].join("\n\n");
  }
  if (unit === "sentences") {
    const matches = [...text.matchAll(/[.!?](?=\s|$)/g)];
    if (newAmount < oldAmount) return text.slice(0, (matches[newAmount - 1]?.index ?? text.length) + 1).trimEnd();
    const added = generateLorem("sentences", newAmount - oldAmount, false, sentenceLength).match(/[^.!?]+[.!?]/g) ?? [];
    return added.reduce((current, sentence, index) =>
      current + ((oldAmount + index) % 4 === 0 ? "\n\n" : " ") + sentence.trim(), text);
  }
  if (unit === "words") {
    if (newAmount > oldAmount) return text + "\n\n" + generateLorem("words", newAmount - oldAmount, false, sentenceLength);
    const words = [...text.matchAll(/\S+/g)];
    const last = words[newAmount - 1];
    if (!last) return generateLorem(unit, newAmount, start, sentenceLength);
    const end = (last.index ?? 0) + last[0].length;
    const trimmed = text.slice(0, end).trimEnd();
    return /[.!?]$/.test(trimmed) ? trimmed : trimmed + ".";
  }
  if (newAmount > oldAmount && newAmount - oldAmount > 8) {
    const additional = generateLorem("characters", newAmount - oldAmount - 1, false, sentenceLength);
    if (additional.length === newAmount - oldAmount - 1) return text + " " + additional;
  }
  return generateLorem(unit, newAmount, start, sentenceLength);
}
