...
function MatchTool() {
  ...
  return <article className="tool-card"><ToolHead title="Match existing text" note="Replace copy while keeping a similar text footprint." />
    ...
    <div className="tool-actions"><Button variant="outline" disabled={!source.trim()} onClick={() => match("characters")}>Match character count</Button><Button variant="outline" disabled={!source.trim()} onClick={() => match("words")}>Match word count</Button><Button variant="outline" disabled={!source.trim()} onClick={() => match("paragraphs")}>Match paragraph count</Button></div>
...
function ExpansionTool() {
  ...
  return <article className="tool-card"><ToolHead title="Text expansion" note="See how a layout behaves with more or less copy." />
    <label className="field-label" htmlFor="expand-source">Source text</label><Textarea id="expand-source" className="tool-textarea" placeholder="Paste text to expand or reduce…" value={source} onChange={e => setSource(e.target.value)} />
    ...
    <Button className="tool-generate" disabled={!source.trim()} onClick={() => setResult(generateLorem("characters", target, false))}>Generate variant <ArrowUpRight aria-hidden="true" /></Button>
...
export default function Home() {
  ...
  return <main className="site-shell" lang="en">
    <div className="page-content">
      ...
      <section className="seo-content" aria-labelledby="why-title"><div className="seo-content-heading"><span className="eyebrow">BUILT FOR REAL LAYOUTS</span><h2 id="why-title">A Lorem Ipsum Generator for designers and developers.</h2></div><div className="seo-content-grid"><article><h3>Keep the layout honest.</h3><p>Generate Lorem Ipsum with exact words and paragraphs together, then adjust the density without rebuilding your settings. Use Characters or Sentences when a component, headline, or text block needs a precise footprint.</p><p>This generator is made for wireframes, editorial layouts, interface prototypes, and type studies. Fit to Box helps you test how copy behaves before final content arrives.</p></article><article><h3>Copy the format your workflow needs.</h3><p>Copy plain text for notes and drafts, or use <strong>Copy HTML</strong> when you are moving placeholder content into a page or component. Switch formats as your layout moves from exploration to implementation.</p><p>Use the result as temporary content during design. Replace it with approved final copy before publishing a finished project.</p></article></div></section>
